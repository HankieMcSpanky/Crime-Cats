...
func _apply_material() -> void:
	...
	body_count = result[0]
	eye_count = result[1]
	print("CatMaterialApplier: Applied body material to ", body_count, " meshes, eye material to ", eye_count, " meshes")
...
func _apply_material_recursive(node: Node) -> Array:
	...
	if node is MeshInstance3D:
		...
		if "eye" in node_name or "cornea" in node_name or "iris" in node_name or "pupil" in node_name:
			...
			eye_count += 1
			print("Found eye mesh: ", node.name)
		else:
