...
static func apply(editor_interface: EditorInterface) -> void:
	_closed_docks.clear()
	_walk_tree(editor_interface.get_base_control(), 0, true, editor_interface)


static func restore(editor_interface: EditorInterface) -> void:
	_walk_tree(editor_interface.get_base_control(), 0, false, editor_interface)

	for dock in _closed_docks:
		if is_instance_valid(dock):
			editor_interface.open_dock(dock)
	_closed_docks.clear()


static func _walk_tree(node: Node, depth: int, hiding: bool, editor_interface: EditorInterface) -> void:
	if depth > 20:
		return

	_process_node(node, hiding, editor_interface)

	for child in node.get_children():
		_walk_tree(child, depth + 1, hiding, editor_interface)


static func _process_node(node: Node, hiding: bool, editor_interface: EditorInterface) -> void:
	...
	if hiding and node.get_class() in ["NodeDock", "HistoryDock"]:
		if node not in _closed_docks:
			...
			editor_interface.close_dock(node)
		return
