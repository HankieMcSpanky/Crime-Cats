...
func _execute_reimport_file(params: Dictionary) -> String:
	...
	if config_files.size() > 0:
		...
		for file_path in config_files:
			Log.info("  - %s" % file_path)
		editor_interface.reload_project_settings()
...
func _execute_editor_output(params: Dictionary) -> String:
	...
	var editor_interface = EditorScript.new().get_editor_interface()
	if not editor_interface:
		return JSON.stringify({"success": false, "error": "Could not get editor interface"})

	var editor_log = _find_editor_log(editor_interface.get_base_control())
	if not editor_log:
