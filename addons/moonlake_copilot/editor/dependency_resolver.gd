...
func initialize(editor_iface, placeholder_mgr, plugin) -> void:
...
func _trigger_import(tscn_path: String) -> void:
	...
	if not editor_interface:
		Log.error("[MOONLAKE] ERROR: EditorInterface not available")
		return
	...
	if editor_interface:
		...
		editor_interface.open_scene_from_path(tscn_path, false, true)  # ignore_broken_deps = true
