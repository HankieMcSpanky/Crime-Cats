...
func _enter_tree() -> void:
	...
	if PublishDialogScript:
		...
		get_editor_interface().get_base_control().add_child(publish_dialog)
	...
	placeholder_manager = PlaceholderManager.new(get_editor_interface())

	var DependencyResolver = load("res://addons/moonlake_copilot/editor/dependency_resolver.gd")
	dependency_resolver = DependencyResolver.new()
	dependency_resolver.initialize(get_editor_interface(), placeholder_manager, self)
	...
	terrain_controller.initialize(get_editor_interface(), python_bridge, self)
	...
	viewport_context_controller.initialize(get_editor_interface(), chat_panel, self)
...
func _apply_ui_mode() -> void:
	...
	if simplified:
		...
		UISimplifier.apply(get_editor_interface())
...
func _on_distraction_free_mode_changed(enabled: bool) -> void:
	...
	if enabled:
		...
		UISimplifier.restore(get_editor_interface())
...
func _setup_moonlake_menu() -> void:
	...
	var menu_bar = _find_menu_bar(get_editor_interface().get_base_control())
	if menu_bar:
...
func _trigger_manual_import_for_current_scene() -> void:
	...
	if not editor_interface:
		_show_alert("Moonlake Import Error", "EditorInterface not available")
		return
...
func _on_create_character_pressed() -> void:
	...
	if paint_scene:
		...
		get_editor_interface().get_base_control().add_child(paint_window)
