...
static func reload_all_modified_scenes() -> String:
	...
	if not editor_interface:
		return "Error: Could not get editor interface"
	...
	editor_interface.reload_modified_scenes()
...
static func open_main_scene_or_fallback(scene_path: String) -> String:
	...
	var open_scenes = Array(editor_interface.get_open_scenes()).map(func(p): return ResourceUID.ensure_path(p))
	...
	if main_scene in open_scenes:
		if active_scene_path == main_scene:
			...
			editor_interface.reload_scene_from_path(main_scene)
			await Engine.get_main_loop().process_frame
			editor_interface.open_scene_from_path(main_scene)
