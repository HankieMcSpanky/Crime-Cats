...
func initialize(editor_iface, chat_pnl, plugin) -> void:
...
func _create_context_popup() -> void:
	...
	swap_model_button.icon = editor_interface.get_base_control().get_theme_icon("Reload", "EditorIcons")
	...
	context_send_button.icon = editor_interface.get_base_control().get_theme_icon("ArrowUp", "EditorIcons")
	...
	editor_interface.get_base_control().add_child(context_popup)
...
func show_asset_selection_popup() -> void:
	...
	if not editor_interface or not selected_node:
		return
	...
	editor_interface.get_base_control().add_child(asset_selection_popup)
...
func _on_asset_selected(glb_path: String) -> void:
	if not editor_interface or not plugin_ref:
		Log.error("[MOONLAKE] Missing editor interface or plugin reference")
		return
