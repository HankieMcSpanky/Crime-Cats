...
func _get_editor_log_messages() -> PackedStringArray:
	var editor_log = _find_editor_log(EditorScript.new().get_editor_interface().get_base_control())
...
func _capture_editor_screenshot() -> Image:
	var image = EditorScript.new().get_editor_interface().get_base_control().get_viewport().get_texture().get_image()
